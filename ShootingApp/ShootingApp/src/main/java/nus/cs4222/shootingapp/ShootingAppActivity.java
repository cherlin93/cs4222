...
public class ShootingAppActivity 
    ...
    private void startSensing() {
        ...
        sensorManager.registerListener( this ,                              // Listener
                                        ...
                                        SensorManager.SENSOR_DELAY_GAME);   // Measurement interval
    }
    ...
    public void onSensorChanged( SensorEvent event ) {
        ...
        // ADDED:
            ...
            processRotationValues( event );
        }
        detectShootingDirectionAndRegion( event );
    }

    /** Process the rotation sensor. */
    /** The idea to use these two APIs in particular is due to the statement found in the SensorManager description of TYPE_ORIENTATION, where the following was mentioned:
    ...
    // ADDED:
    private void processRotationValues( SensorEvent event ) {
        // Getting rotation matrix from the sensor
        float[] rotationMatrix = new float[9];
        SensorManager.getRotationMatrixFromVector(rotationMatrix, event.values);

        // Getting rotation values from the rotation matrix
        float[] rotationValues = new float[3];
        SensorManager.getOrientation(rotationMatrix, rotationValues);

        // Getting the degree of the position that the user is pointing phone in
        rotationValues[0] = (float) Math.toDegrees(rotationValues[0]); // since value is in radians

        // Printing to GUI
        shootingDirection = rotationValues[0] + 180;
