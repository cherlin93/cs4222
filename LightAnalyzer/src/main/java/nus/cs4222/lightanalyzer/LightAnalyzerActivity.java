...
public class LightAnalyzerActivity 
    ...
    public void onSensorChanged( SensorEvent event ) {
        ...
        updateLocationView(lux);
        updateLatitudeLongitudeView(isOutdoors);
    ...
    // ADDED: Helper method that updates the longitude and latitude text view. */
    private void updateLatitudeLongitudeView(Boolean isOutdoors) {
        final StringBuilder sb = new StringBuilder();
        if(isOutdoors) { //Need to turn on GPS
            if(!locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)) { //GPS IS NOT ON
            ...
            sb.append("Location: INDOORS");
        }
        // Update the location view in the main UI thread
    ...
    // ADEED: For requesting of permission to turn on GPS
    ...
    // ADDED: Helper method to check if location permission has been granted
    ...
    // ADDED: Helper method that updates the location text view. */
    private void updateLocationView(float lux) {
        final StringBuilder sb = new StringBuilder();
        sb.append("Location: ");
        if(lux <= INDOOR_THRESHOLD) {
            sb.append("INDOORS");
            isOutdoors = false;
        } else {
            sb.append("OUTDOORS");
    ...
    /** ADDED: Necessary overrides for location manager API. */
    @Override
